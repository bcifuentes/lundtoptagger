...
#create_train_dataset_fulld_new_Ntrk_pt_weight_file_test( dataset, graph_small_example , all_lund_zs, all_lund_kts, all_lund_drs, parent1, parent2, flat_weights, labels ,N_tracks,jet_pts, jet_ms, kT_selection, mcweights,mcweights_out, Good_jets)
    ...
    #print(graphs)
    return graphs
def create_train_dataset_fulld_new_Ntrk_pt_weight_file_test_export(graphs, graph_small_example, z, k, d, edge1, edge2, weight, label, Ntracks, jet_pts, jet_ms, kT_selection, mcweights, mcweights_out, Good_jets, signal_jet_truth_label):
...
def create_train_dataset_fulld_new_Ntrk_pt_weight_file_test_onnx(graphs, graph_small_example, z, k, d, edge1, edge2, weight, label, Ntracks, jet_pts, jet_ms, kT_selection, mcweights, mcweights_out, Good_jets, signal_jet_truth_label):
    ...
    for i in range(len(z)):  
        ...
        onnx_input = {
            "x": vec.detach().numpy(),  # Características de los nodos: d_out, z_out, k_out
            "edge_index": edge.detach().numpy(),  # Índices de los bordes del grafo
            "Ntrk": Ntrk
        }
        
        graphs.append(onnx_input)
    ...
    #print(graphs)
    return graphs
def onnx_data_loader(dataset, batch_size, shuffle_batches=False):
    ...
    for i in range(0, n, batch_size):
        ...
        xs        = [dataset[j]["x"]          for j in batch_idx]
        edge_idxs = [dataset[j]["edge_index"] for j in batch_idx]
        Ntrks     = [dataset[j]["Ntrk"]       for j in batch_idx]
        
        if len(xs) < batch_size:
            last_x = xs[-1]
            last_edge = edge_idxs[-1]
            last_Ntrk = Ntrks[-1]

            pad = batch_size - len(xs)
            xs += [last_x] * pad
            edge_idxs += [last_edge] * pad
            Ntrks += [last_Ntrk] * pad

        counts = np.array([int(x.shape[0]) for x in xs], dtype=np.int64)   # <- INT64 explicit


        offsets = np.concatenate([[0], np.cumsum(counts, dtype=np.int64)[:-1]]).astype(np.int64)


        x_concat = np.concatenate(xs, axis=0).astype(np.float32)

        adjusted_edges = []
        for off, e in zip(offsets, edge_idxs):
            ...
            if e.ndim != 2 or e.shape[0] != 2:
                raise ValueError(f"edge_index individual con shape inválido: {e.shape}")
            adjusted_edges.append((e + int(off)).astype(np.int64))

        if len(adjusted_edges) > 0:
            edge_index_concat = np.concatenate(adjusted_edges, axis=1).astype(np.int64)
        else:
            edge_index_concat = np.zeros((2, 0), dtype=np.int64)

        batch_vector = np.concatenate([
            np.full(int(counts[k]), k, dtype=np.int64)
        ...
        yield {
            ...
            "edge_index": edge_index_concat, # int64 [2, total_edges]
            "batch":      batch_vector,      # int64 [total_nodes]
            "Ntrk":       np.array(Ntrks, dtype=np.float32),  # float32 [batch_size]
            "counts":     counts             # int64 [batch_size]
        }
def get_scores_multi(loader, model, device):
    model.eval()
    outputs = []   # lista de batches
    ...
    # concatena todo al final
    total_output = np.concatenate(outputs, axis=0)
    return total_output
def evaluate_onnx(test_loader_onnx, model, batch_size, total_samples):
    tagger_scores = []
    seq2=[]
    batch_counter = 0
    for batch_data in test_loader_onnx:
        # Obtener los datos del lote (siguen siendo tensores de PyTorch)
        batch_counter += 1
        x = batch_data["x"]
        edge_index = batch_data["edge_index"]
        Ntrk = batch_data["Ntrk"]
        counts = batch_data["counts"]
        batch = batch_data["batch"]

        onnx_input = {
            'x': x.astype(np.float32),          # Clave corregida, tipo float32
            'edge_index': edge_index.astype(np.int64), # Tipo int64
            'batch': batch.astype(np.int64),           # Tipo int64
            'Ntrk': Ntrk.astype(np.float32),         # Tipo float32
            #"counts": counts.astype(np.int64) #
        }

        if batch_counter == 1:  # solo imprimir el primer batch
            print("---- ONNX DEBUG ----")
            for k, v in onnx_input.items():
                print(f"{k}: shape={v.shape}, dtype={v.dtype}, mean={v.mean() if v.ndim>0 else v}")
            print("---------------------")
        # Ejecutar la inferencia en ONNX
        output = model.run(None, onnx_input)
        #s2 = output[6]
        # El resto de tu lógica para procesar la salida parece correcta
        jet_scores = output[0]
        tagger_scores.append(jet_scores.flatten())
        #seq2.append(s2)
        
        #if batch_counter == 1:
        #    print("\n🔎 Stats ONNX intermedios (primer batch):")
        #    for idx, name in enumerate(["final", "conv1","conv2","conv3","conv4","conv5", "conv6","finallinear","x_cat", "seq1", "x_global","x_ntrk","seq2"]):
        #        arr = output[idx]
        #        print(f"{name}: shape={arr.shape}, mean={arr.mean():.4f}, std={arr.std():.4f}, "
        #              f"min={arr.min():.4f}, max={arr.max():.4f}")
             
    # Convertir los resultados a un array de numpy para su posterior procesamiento
    #tagger_scores = np.concatenate(tagger_scores, axis=0)
    #print("len(seq2onnx)=",len(seq2[0]))
    #return tagger_scores,seq2[0]
    
    tagger_scores = np.concatenate(tagger_scores, axis=0)
    #seq2_all = seq2[0]
    
    # 🧩 Recorta a tamaño real
    #true_len = len(dataset)
    tagger_scores = tagger_scores[:total_samples]
    #seq2_all = seq2_all[:total_samples]
    return tagger_scores    
    
def evaluate_onnx_multi(test_loader_onnx, model):
    tagger_scores = []
    ...
    for batch_data in test_loader_onnx:
        ...
        onnx_input = {
            ...
            "Ntrk": Ntrk.astype(np.float32),
        }

        output = model.run(None, onnx_input)

        jet_scores = output[0]  
        # shape esperado: (n_jets_batch, n_classes)

        tagger_scores.append(jet_scores)
...
def get_scores_manual(loader, model, device):
    ...
    for data in loader:
        ...
        data = data.to(device)
        counts = torch.bincount(data.batch)
        pred = model(data.x, data.edge_index, data.batch, data.Ntrk, counts)
        total_output = np.append(total_output, pred.cpu().detach().numpy(), axis=0)
        #s2 = model._saved_seq2  
        #seq2.append(s2)
        # 🔎 Ver stats de las capas intermedias (solo primer batch)
        #if batch_counter == 1:
        #    print("\n🔎 Stats PyTorch intermedios (primer batch):")
        #    for k, v in model.debug_outputs.items():
        #        print(f"{k}: {v}")
                
    #print("len(s2torch)=",len(seq2[0]))
    return total_output[1:]
